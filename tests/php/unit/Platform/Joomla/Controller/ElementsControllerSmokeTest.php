<?php
/**
 * ElementsController smoke-test — structural & dispatch contracts.
 *
 * Round-4 audit A3 P1. See {@see PagesControllerSmokeTest} class
 * docblock for the smoke-test rationale (Joomla MVCFactory autoload
 * → file-grep contract pins).
 *
...
final class ElementsControllerSmokeTest extends TestCase
{
    private const REL_PATH = 'src/packaging/joomla/extensions/com_ytbmcp/api/src/Controller/ElementsController.php';
    ...
    public function test_extends_abstract_api_controller(): void
    {
        self::assertMatchesRegularExpression(
            '/final\s+class\s+ElementsController\s+extends\s+AbstractApiController\b/',
            $this->controllerSource(),
            'ElementsController must extend AbstractApiController (auth-stack pin).'
        );
    }
    ...
    public function test_method_dispatches_with_correct_scope(string $method, string $expectedScope): void
    {
        self::assertStringContainsString(
            ...
            "ElementsController::$method() must dispatch with scope '$expectedScope'."
        );
    }
    ...
    public function test_cache_flush_helper_calls_flush_l1(): void
    {
        ...
        self::assertDoesNotMatchRegularExpression(
            '/->\s*flush\s*\(\s*\)/',
            $stripped,
            'ElementsController must NOT call the dead ->flush() API.'
        );
    }
    ...
    public static function methodScopeProvider(): iterable
    {
        ...
        yield 'get'            => ['get',            'read'];
        yield 'add'            => ['add',            'write'];
        yield 'updateSettings' => ['updateSettings', 'write'];
        yield 'delete'         => ['delete',         'write'];
        yield 'move'           => ['move',           'write'];
        yield 'clone'          => ['clone',          'write'];
    }
    ...
    private function methodBody(string $method): string
    {
        ...
        if (!\preg_match(
            ...
            self::fail("Could not locate method body for $method in ElementsController.");
        }
        return $m[1];
    }
}
