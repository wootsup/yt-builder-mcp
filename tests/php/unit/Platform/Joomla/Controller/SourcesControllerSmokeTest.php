<?php
/**
 * SourcesController smoke-test — structural & dispatch contracts.
 *
...
final class SourcesControllerSmokeTest extends TestCase
{
    private const REL_PATH = 'src/packaging/joomla/extensions/com_ytbmcp/api/src/Controller/SourcesController.php';
    ...
    public function test_extends_abstract_api_controller(): void
    {
        self::assertMatchesRegularExpression(
            '/final\s+class\s+SourcesController\s+extends\s+AbstractApiController\b/',
            $this->controllerSource(),
            'SourcesController must extend AbstractApiController (auth-stack pin).'
        );
    }
    ...
    public function test_method_dispatches_with_correct_scope(string $method, string $expectedScope): void
    {
        self::assertStringContainsString(
            ...
            "SourcesController::$method() must dispatch with scope '$expectedScope'."
        );
    }
    ...
    public function test_cache_flush_helper_calls_flush_l1(): void
    {
        ...
        self::assertDoesNotMatchRegularExpression(
            '/->\s*flush\s*\(\s*\)/',
            $stripped,
            'SourcesController must NOT call the dead ->flush() API.'
        );
    }
    ...
    private function methodBody(string $method): string
    {
        ...
        if (!\preg_match(
            ...
            self::fail("Could not locate method body for $method in SourcesController.");
        }
        return $m[1];
    }
}
