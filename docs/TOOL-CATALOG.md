...
| `yootheme_builder_element_types_list` | read+openWorld | `fields` | List element types registered on this site (built-ins + any third-party extras a site has installed). Names feed `element_type` of element_add. Pass `fields[]` to narrow each row. |
