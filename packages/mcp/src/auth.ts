 ...
 *                           Settings page (`wp-admin → YOOtheme Builder MCP`).
 *
...
const BEARER_FORMAT_SCHEMA = z
    ...
    .max(
        BEARER_MAX_LEN,
        `Bearer too long (max ${String(BEARER_MAX_LEN)} chars). The env var YTB_MCP_BEARER_TOKEN seems to contain something other than a plain key — strip leading "Bearer " or paste only the key copied from wp-admin → "YOOtheme Builder MCP" → Settings.`,
    )
    .regex(
        /^ytb_(live|test)_[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/,
        'Bearer must match ytb_(live|test)_<payloadB64Url>.<sigB64Url> format (copy the key as shown in wp-admin → "YOOtheme Builder MCP" → Settings; do NOT prepend "Bearer ").',
    );
...
export function loadConfig(options: LoadConfigOptions = {}): ServerConfig {
    ...
    if (!options.allowMissingToken && rawToken === '') {
        throw new ConfigError(
            `Missing ${ENV_BEARER}. Generate a Bearer key in wp-admin → "YOOtheme Builder MCP" → ` +
