...
//
// L1 + direct + gateway = 10 surface tools (visible in tools/list).
// L2 = 11 advanced tools (invoked via the gateway).
//
...
const EXPECTED_TOTAL = SURFACE_TOOLS.length + ADVANCED_TOOLS.length;
...
async function runVerification(token, tokenSource) {
    ...
    try {
        ...
        for (const spec of SURFACE_TOOLS) {
            const resp = await client.callTool(spec.name, spec.sampleArgs);
            const { status, detail } = classifyToolResult(resp, spec);
            results.push({ name: spec.name, kind: spec.kind, lane: 'surface', status, detail });
        }

        for (const spec of ADVANCED_TOOLS) {
            // Route via gateway
            const resp = await client.callTool('yootheme_builder_advanced', {
                tool: spec.name,
                arguments: spec.sampleArgs,
            });
            const { status, detail } = classifyToolResult(resp, spec);
            results.push({ name: spec.name, kind: spec.kind, lane: 'advanced', status, detail });
        }
...
function renderReport({ advertisedNames, results, token, tokenSource, error }) {
    ...
    lines.push('');
    lines.push(`- **Server-advertised tools/list count:** ${String(advertisedNames.length)} (expected 10)`);
    lines.push(`- **Total registered tools (incl. advanced):** ${String(EXPECTED_TOTAL)} (expected ${String(EXPECTED_TOTAL)})`);
    lines.push('');

    lines.push('## Surface tools (tools/list — 10 entries)');
    lines.push('');
