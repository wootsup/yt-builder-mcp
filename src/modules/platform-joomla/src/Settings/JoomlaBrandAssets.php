<?php
/**
 * JoomlaBrandAssets — WootsUp brand primitives for the com_ytbmcp admin UI.
 *
 * Joomla-side twin of {@see \WootsUp\BuilderMcp\Platform\WordPress\BrandAssets}.
 * The SVG paths + palette + inline-style block are byte-identical to the WP
 * surface so the two admin pages render the same brand lock-up (cookbook §6
 * verbatim rule). Both methods are pure string builders — no Joomla / WP
 * runtime symbols — so this class stays under full PHPStan level-8 scrutiny
 * (unlike the `src/packaging/joomla/*` view layer which is excludePaths-scoped).
 *
 * Kept here (platform-joomla module) rather than reusing the platform-wordpress
 * `BrandAssets` so the Joomla packaging tree carries NO dependency on the WP
 * module — the two platform adapters stay independently shippable.
 *
 * @package    WootsUp\BuilderMcp\Platform\Joomla\Settings
...
final class JoomlaBrandAssets
{
     ...
     *
     * Paths preserved verbatim from the api-mapper React source
     * (WootsUpLogo.tsx); only the numeric `size` attr is variable, so direct
     * echo of the return value is safe (no user-controllable input).
     *
    ...
    /**
     * Return the inline `<style>` block for the com_ytbmcp admin surfaces.
     *
     * W11 (2026-05-24): the admin UI was redesigned to render as a NATIVE
     * Joomla component (native toolbar + Bootstrap/uitab/Atum components). The
     * bespoke brand surface — the former inline `<style>` block AND its W10-T2
     * successor (`media/com_ytbmcp/css/admin.css`, registered via the
     * WebAssetManager) — was removed entirely. Because every body component is
     * now native Atum-themed, the dashboard follows Joomla's light + dark admin
     * theme for free, with zero custom colour CSS. The ONLY brand element kept
     * is the logo SVG ({@see self::renderLogo()}).
     *
     ...
     * instead of a fatal).
     *
     * @deprecated W11 — styling dropped; the admin UI is now native Joomla/Bootstrap.
     */
